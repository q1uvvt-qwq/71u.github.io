...
直接发一个带空格的条件（例如 `name=admin or 1=1`）会**直接报错**。原笔记的结论就一句：

> 简单发送后显示错误说明有过滤

报错而不是回显两种文案，说明空格这类字符在**进入 SQL 之前**就被拦掉了 —— 后端的过滤规则认得出普通空格（或普通 payload 里的关键词），但我们的目标是让拼出来的 SQL 语法仍然合法。于是改用 `/**/` 代替空格：
...
到这里，「一个回显可控的布尔开关」就做好了：**任何能写成 SQL 逻辑表达式的条件，都可以塞进 `or` 后面，用回显文案判断真假**。这就是盲注能成立的全部前提。

### 第三步：把要读的东西写成「条件」

布尔盲注本身不区分数据库，关键是要有能返回**单个值**的表达式。SQLite 里常用：

- `substr(表达式, 位置, 长度)` —— 取子串
- `unicode(表达式)` —— 取字符编码，便于二分
- `sqlite_master` —— 存放所有**表结构的元数据表**（这正对应原文要求的「去模板表名」：表名不必猜，先盲注它读出来）
...
回显「该用户存在且活跃」→ 第 1 个字符就是 `f`；否则换下一个候选字符继续试。
...
> **无空格过滤绕过（`/**/` 顶替空格）** → 用 `or <条件>` 把真假映射成两种回显文案 → 条件里用 `substr()` / `unicode()` 逐字符取值 → 先用 `sqlite_master` 盲注出表名列名（「去模板表名」），再读数据。
...
`or 1=1` 让 `where` 恒成立，于是「查到记录」；把 `1=1` 换成 `1=2` 就是「查不到」。应用层分别给这两种情况输出了不同文案，回显就此变成 `where` 是否命中的信号位。

### 2. 从「二元信号」到「读出字符串」

有了信号位之后，读数据全靠**把要读的东西写成条件**：
...
> 这一步是**按题面和 SQLite 通用能力推导出来的**，不是账号里的现成脚本 —— 原笔记只给了发包和回显，没有留下 flag 值或实测记录。

### 3. SQLite 环境与 MySQL 的差异（原笔记能得出的部分）

原笔记只覆盖了本题实际用到的两点，其余仅作补充说明，避免过度推广：

- **`/**/` 可当空格**：这是本题绕过过滤的关键，与 MySQL 里 `/**/` 也能当内联注释/分隔符的用法一致。
- **`--+` 收尾**：`+` 在 URL 里被解码为空格，于是变成 `-- `，把原 SQL 后半段注释掉。
- **补充（原笔记未展开）**：SQLite **没有 MySQL 的 `information_schema`**，元数据统一放在 `sqlite_master`（较新版本别名 `sqlite_schema`）里，字段为 `type` / `name` / `tbl_name` / `sql` 等。本题「去模板表名」正是靠它：先盲注出 `sqlite_master.sql`，从中读出真实表名与列名，再按名字去读数据。**这一点是 SQLite 的通用常识，不是这道题的实测结论**，仅供参考。

## 完整示例

下面是一份按本题形态推导的**布尔盲注脚本模板**，用 `requests` 根据 True/False 回显差异逐字符取值。

```python
import requests

BASE = "http://靶机地址/check.php"          # 原笔记中为一串带 UUID 的一次性域名，已抽象
...
if __name__ == "__main__":
    ...
    # 1) “去模板表名”：先把 sqlite_master 里的建表语句盲注出来，
...
- **恒真/恒假先自检**：`assert inject("1=1")` 和 `inject("1=2")` 是用来确认「布尔信号仍然成立」的。如果这一步就挂了，后面所有猜解结果都不可信。
- **先读 `sqlite_master` 再读数据**：这就是题目要求「去模板表名」的落地方式 —— 不猜表名，让数据库自己把建表语句交出来。
- **线性枚举换二分**：模板为了好读用了逐字符枚举（最坏一个字符 `len(CHARSET)` 次请求）。数据量大时把条件换成 `unicode(substr(...))>{mid}` 做二分，每字符约 7 次请求即可。

## 踩坑与备注

- **脚本为按题面推导的模板，未在靶场实测**：原笔记只留下了发包、回显文案和「去模板表名」的要求，**没有给出最终 flag 值，也没有留下跑通的脚本回显**。上面这份脚本是按这些信息推导出的通用框架，实际使用时需要根据靶场的过滤规则和回显文案微调。
- **回显文案以靶场实际为准**：原笔记记录的真分支文案写作「该用户存在**自**活跃」，从上下文看应为「且活跃」（假分支是「未找到用户或已停用」，两句对仗）。脚本里的 `TRUE_TEXT` 请按页面实际返回核对，别照抄。
- **判断回显优先用「点不动」的特征**：如果两种状态的**响应长度**差异明显，用 `len(r.text)` 比较比匹配中文更抗干扰；中文文案若被 `\uXXXX` 转义，`requests` 会自动解码，直接比中文即可。
...
- **SQLite 不区分「库」**：一个文件就是一个库，没有 `database()` 这种切库概念，元数据只有 `sqlite_master`（或 `sqlite_schema`）。
