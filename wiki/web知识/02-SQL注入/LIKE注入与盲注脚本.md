...
`LIKE` 是 SQL 的模糊匹配运算符，配合通配符 `%`（任意长度）和 `_`（单个字符）做前缀/包含匹配。在注入里它有两个价值：**顶替被过滤的 `=`**，以及**用「已猜前缀 + `%`」逐字符爆破**。
...
`pass LIKE 0x63746625`（即 `'ctf%'`）只判断「是否存在以 `ctf` 开头的密码」。把它扩展成「已猜出的整段前缀 + 新字符 + `%`」，就能一次确认这一位猜得对不对——比 `substr()` 逐位比较省一半请求。
...
2. **`like` 代替 `=`**：`=` 被拉黑，但 `like` 没被拉黑，而 `like '%ctfshow%'` 本身就是一次"包含"匹配；
...
- 模板里的 `ctfshow` 是已知固定前缀，`{}` 处填入「已猜部分 + 新字符」；
- 判据是页面里出现 `$user_count = 1;`——匹配到的记录数为 1 说明这个字符猜对了；
- `regexp()` 与 `like` 效果类似但支持正则，`^ctfshow{a` 这种前缀锚定写法更精确。

### 注入手法一句话总结

> **空格被拦用反引号代替、`=` 被拦用 `like`/`regexp` 代替，把 `tableName` 拼成 `\`ctfshow_user\`where\`pass\`regexp("...")`，再用「已猜前缀 + 新字符」做前缀匹配逐位爆破。**

## 通用布尔盲注脚本（附）

下面是同一题型下常用的**二分法布尔盲注**脚本，目标是一个 `id` 参数真实参与 SQL 的接口：
...
2. 条件为真 → 目标字符在右半区，`low = mid + 1`；否则在左半区，`high = mid`；
...
- **反引号能当空格用但只对标识符有效**：`` `pass` `` 可以，`pass` 和 `like` 之间没有空格靠反引号分隔；但数字/字符串之间仍需另想办法（`/**/`、`%0b` 等）。
